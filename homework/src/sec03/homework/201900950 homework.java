...
interface Animal { public abstract void cry(); } //interface를 여러개 구현하였으며 그중 입력에 나오는 

interface Pet { public abstract void play(); } //결과값을 위해서 다형성 interface를 사용하였습니다.



class Cat implements Animal, Pet {

    public void cry() { //한류의 목록중 역사에 대한 부분의 결과 값이다.
    ...
    public void play() { //한류의 목록 중 종류에 관한 부분의 결과 값이다.
...
class Dog implements Animal, Pet {
    ...
    public void cry() { //한류의 목록 중 한류를 대표하는 유명인에 대한 부분의 결과 값이다.
    ...
    public void play() { //한류의 목록 중 한류의 전망에 관한 부분의 결과 값이다.
...
public class Polymorphism04 {
    ...
    public static void main(String[] args) throws IOException {
		String str = "한류:\n한류는 좁게는 음악, 영화, 드라마 같은 대중문화부터 시작하여 넓게는 패션, 화장품, 음식, 관광, 무술, 산업 등" //실행을 하였을 때 바로 나오는 문장 출력 값
				+ "대한민국의 문화가 세계에 알려지는 현상을 뜻한다.";
		Scanner sc = new Scanner(System.in); 
		System.out.println(str); 
    	FileWriter fw = new FileWriter("c:/out.txt"); //느낀점을 쓸 메모장 생성
		String title = "목차";
		String title1 = null; 
    	...
    	int subtitle1 = 0; 
		int i = 1;
		title1 = sc.nextLine(); //기계인지 아닌지 확인 
		while(true) { //만약 목차를 입력하라고 하였는데 다른 걸 입력였다면 다시 되돌아가서 입력하라고 출력하기 목차라고 입력할 때 까지 무한 반복
		...
		if (title1.equals(title)) { //목차를 맡게 적었다면 참조타입의 열거 타입으로 만들어놓은 subtitle에서 목차 값 출력하기
		...
		subtitle Main = null;
		Scanner sc1 = new Scanner(System.in); //원하는 목차 값 부분 고르기(목차 값을 출력할 때 앞에 숫자로 표시하여서 숫자를 보고 숫자를 치기
        System.out.println("보기에 원하시는 부분의 숫자를 입력해주세요. 6번을 누르시면 프로그램이 종료됩니다.");
        Scanner sc3 = new Scanner(System.in); 
		try { //만약 숫자가 아닌 다른걸 입력했다면 예외처리를 하여서 1로 입력하기
		...
		switch(subtitle1) { //열거 타입의 원하는 입력 값을 위한 스위치 함수 적용
			case 1:
    	...
    	System.out.println(Main); //입력 받은 subtitle1 값을 출력하기
    	
    	Cat c = new Cat(); //다형성 interface로 입력된 값 넣기

        Dog d = new Dog();
        if (subtitle1 == 1) { //만약 1이 입력됬다면 1에 대한 내용을 보여주고 다른부분을 볼 수 있으므로 다시 입력받기
            c.cry();  
            System.out.println("다른부분을 보기 원하신다면 숫자를 입력해주세요.");
			subtitle1 = sc3.nextInt();
    		switch(subtitle1) {
			case 1:
				Main = subtitle.HISTORY; break;
			case 2:
        ...
        if (subtitle1 == 2) { //만약 2가 입력됬다면 2에 대한 내용을 보여주고 다른부분을 볼 수 있으므로 다시 입력받기
        	c.play();
            System.out.println("다른부분을 보기 원하신다면 숫자를 입력해주세요.");
        	subtitle1 = sc3.nextInt();
    		switch(subtitle1) {
        ...
        if (subtitle1 == 3) { //만약 3이 입력됬다면 3에 대한 내용을 보여주고 다른부분을 볼 수 있으므로 다시 입력받기
        	d.cry();
            System.out.println("다른부분을 보기 원하신다면 숫자를 입력해주세요.");
        	subtitle1 = sc3.nextInt();
    		switch(subtitle1) {
        ...
        if (subtitle1 == 4) { //만약 4가 입력됬다면 4에 대한 내용을 보여주고 다른부분을 볼 수 있으므로 다시 입력받기
        	d.play(); 
            System.out.println("다른부분을 보기 원하신다면 숫자를 입력해주세요.");
        	subtitle1 = sc3.nextInt();
    		switch(subtitle1) {
        ...
        if (subtitle1 == 5) { //만약 5가 입력됬다면 처음에 만들었던 txt파일에 내가 느낀점을 적고 엔터를 누르면 입력 받은 값이 txt파일에 저장됩니다.
            System.out.println("한류에 관한 느낀점을 적어주세요."); 
        ...
        if (subtitle1 == 6) { //만약 6이 입력됬다면 프로그램을 종료합니다.
        	System.out.println("프로그램을 종료합니다.");
        }
    }

}
//이번 과제에서 한류에 대하여 어떻게 설명을 해야되는지 고민한 결과 단순히 알아가는 것뿐만이 아닌 느끼고 자신의 생각을 표출 할 수 있는 부분이 있는 프로그램 제작을 생각하였습니다.
//그래서 한류에 관해서 알려주는 것(역사, 종류, 유명인)뿐 만이 아닌 한류의 전망등을 알려주고 스스로 생각하면서 마지막에 느낀점을 적으므로서 한류의 관한 자신의 생각을 정리할 수 있는 프로그램을 제작하게 되었습니다.
//특히 이런부분에서는 집중이 필요하다고 생각하여 웹페이지와 달리 목차를 보고 넘어가기 위해서는 목차를 직접 입력해야하는 수고가 필요합니다. 이부분을 통해 멈춤 사람과 입력을 하려하는 사람으로 나눌수 있다고 생각됩니다.
//그 이후에는 한류에 관한 정보를 알려주고 마지막 전망과 느낀점에서 자신의 생각을 펼치며 정보를 얻는 것만이 아닌 자신의 생각으로 정보를 이해하는 프로그램을 생각하게되었습니다.